            ...
            if(meta->magic == MAGIC_MATCH && meta->msgLength <= 256) {
                ...
                if (response == 0 && transferred == meta->msgLength) {
                    ...
                    for(int i = 0; i < RANGING_MAX_Tr_UNIT; i++) {
                        fprintf(file, "%lu,%u,", rangingMessage.header.lastTxTimestamps[i].timestamp.full % UWB_MAX_TIMESTAMP, rangingMessage.header.lastTxTimestamps[i].seqNumber);
                    }
...
int main() {
    ...
    #if defined(SWARM_RANGING_MODE)
        fprintSwarmRangingMessaage(log_file, device_handle);
    #elif defined(DYNAMIC_SWARM_RANGING_MODE)
        fprintDynamicSwarmRangingMessaage(log_file, device_handle);
    #endif
